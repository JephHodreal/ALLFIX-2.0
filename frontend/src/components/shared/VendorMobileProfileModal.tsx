...
import { X, Building2, Bell, Sun, Moon, Wrench, Users, LogOut, LifeBuoy, ShieldCheck, ChevronRight } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { logoutUser } from '../../services/firebaseService';
import { MobileConfirmModal } from './MobileConfirmModal';

interface VendorMobileProfileModalProps {
  isOpen: boolean;
  onClose: () => void;
  profile: any;
  isDark: boolean;
...
export function VendorMobileProfileModal({
  isOpen,
  onClose,
  profile,
  isDark,
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[9999] lg:hidden pointer-events-auto">
          ...
          >
            ...
            {/* Sheet Header: Vendor Info - Compact Line 1 & Muted Line 2 */}
            <div className="px-6 pt-2 pb-4 border-b border-slate-100 dark:border-slate-800/80 flex items-center justify-between shrink-0 bg-white/80 dark:bg-[#090D16]/80 backdrop-blur-md z-10">
              <div className="flex items-center gap-3.5 min-w-0">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-brand-navy via-indigo-900 to-brand-green p-0.5 shadow-md shrink-0 relative group">
                  <div className="w-full h-full rounded-[14px] bg-white dark:bg-slate-900 overflow-hidden flex items-center justify-center">
                    {profile?.avatar_url || profile?.logo_url ? (
                      <img src={profile.avatar_url || profile.logo_url} alt="Vendor Logo" className="w-full h-full object-cover" />
                    ) : (
                      <span className="text-xl font-black text-brand-navy dark:text-white uppercase">
                        {profile?.company_name?.[0] || profile?.first_name?.[0] || 'V'}
                      </span>
                    )}
                  </div>
                ...
                <div className="min-w-0 flex flex-col justify-center">
                  ...
                  <p className="text-xs font-semibold text-slate-400 dark:text-slate-500 truncate mt-0.5">
                    {profile?.email || 'Authorized Service Provider'}
                  </p>
                </div>
              </div>
              <button
            ...
            {/* Scrollable Navigation Body - Invisible Gliding Scroll */}
            <div className="p-5 overflow-y-auto space-y-5 no-scrollbar [&::-webkit-scrollbar]:hidden flex-1 relative">
              {/* Panel 1: Core Account & Business (Unified Frosted Glass Panel) */}
              <div className="space-y-2">
                ...
                <div className="rounded-3xl bg-slate-50/80 dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/60 dark:border-slate-800/60 overflow-hidden divide-y divide-slate-200/50 dark:divide-slate-800/60 shadow-xs">
                  {/* Company Profile Row */}
                  <button
                    onClick={() => handleNav('/vendor/profile')}
                  ...
                  >
                    <div className="flex items-center gap-3.5 min-w-0">
                      ...
                      <div className="flex flex-col text-left min-w-0">
                        <span className="font-extrabold text-sm sm:text-base text-slate-900 dark:text-white truncate">Company Profile</span>
                        <span className="text-xs text-slate-500 dark:text-slate-400 font-medium truncate">Business details, documents & logo</span>
                      </div>
                    </div>
                  ...
                  {/* Services Row */}
                  <button
                    onClick={() => handleNav('/vendor/services')}
                    className="w-full flex items-center justify-between py-3.5 px-4 transition-all duration-200 hover:bg-slate-100/50 dark:hover:bg-slate-800/40 active:bg-slate-100 dark:active:bg-slate-800 cursor-pointer group"
                  >
                    <div className="flex items-center gap-3.5 min-w-0">
                      ...
                      <div className="flex flex-col text-left min-w-0">
                        <span className="font-extrabold text-sm sm:text-base text-slate-900 dark:text-white truncate">Services & Add-ons</span>
                        <span className="text-xs text-slate-500 dark:text-slate-400 font-medium truncate">Manage service offerings & pricing</span>
                      </div>
                    </div>
                    <ChevronRight className="w-4 h-4 text-slate-400 dark:text-slate-500 shrink-0 ml-2 group-hover:translate-x-0.5 transition-transform" />
                  </button>
                  ...
                  {/* Personnel Row */}
                  <button
                    onClick={() => handleNav('/vendor/personnel')}
                    className="w-full flex items-center justify-between py-3.5 px-4 transition-all duration-200 hover:bg-slate-100/50 dark:hover:bg-slate-800/40 active:bg-slate-100 dark:active:bg-slate-800 cursor-pointer group"
                  >
                    <div className="flex items-center gap-3.5 min-w-0">
                  ...
                  {/* Notifications Row */}
                  <button
                    onClick={() => handleNav('/vendor/notifications')}
                    className="w-full flex items-center justify-between py-3.5 px-4 transition-all duration-200 hover:bg-slate-100/50 dark:hover:bg-slate-800/40 active:bg-slate-100 dark:active:bg-slate-800 cursor-pointer group"
                  >
                    <div className="flex items-center gap-3.5 min-w-0">
              ...
              {/* Panel 2: Preferences & Support (Unified Frosted Glass Panel) */}
              <div className="space-y-2">
                <span className="text-[11px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500 px-2">
                  Preferences & Support
                </span>
                <div className="rounded-3xl bg-slate-50/80 dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/60 dark:border-slate-800/60 overflow-hidden divide-y divide-slate-200/50 dark:divide-slate-800/60 shadow-xs">
                  ...
                  {/* Support Row */}
                  <button
                    onClick={() => handleNav('/vendor/support')}
                    className="w-full flex items-center justify-between py-3.5 px-4 transition-all duration-200 hover:bg-slate-100/50 dark:hover:bg-slate-800/40 active:bg-slate-100 dark:active:bg-slate-800 cursor-pointer group"
                  >
                    <div className="flex items-center gap-3.5 min-w-0">
                      ...
                      <div className="flex flex-col text-left min-w-0">
                        ...
                        <span className="text-xs text-slate-500 dark:text-slate-400 font-medium truncate">24/7 Vendor concierge assistance</span>
                      </div>
                    </div>
                    <ChevronRight className="w-4 h-4 text-slate-400 dark:text-slate-500 shrink-0 ml-2 group-hover:translate-x-0.5 transition-transform" />
                  </button>
            ...
            {/* Sheet Footer: Docked Log Out Button & Symmetry */}
            <div className="p-5 border-t border-slate-100 dark:border-slate-800/80 bg-slate-50/60 dark:bg-slate-900/40 pb-safe space-y-3 shrink-0">
              ...
              <div className="text-center space-y-1 pt-0.5">
                <p className="text-[11px] font-extrabold text-slate-400 dark:text-slate-500 tracking-wide">
                  AllFix Vendor Portal v2.0 • Service Concierge
                </p>
