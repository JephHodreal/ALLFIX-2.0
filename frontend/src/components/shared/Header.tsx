...
import { VendorMobileProfileModal } from './VendorMobileProfileModal';
import { PersonnelMobileProfileModal } from './PersonnelMobileProfileModal';
...
export function Header({ onMenuToggle, title }: HeaderProps) {
  ...
  const [vendorMobileMenuOpen, setVendorMobileMenuOpen] = React.useState(false);
  const [personnelMobileMenuOpen, setPersonnelMobileMenuOpen] = React.useState(false);
  ...
  return (
    <header className="h-16 bg-white dark:bg-[#020617] border-b border-slate-200 dark:border-[#1E293B] flex items-center justify-between px-6 sticky top-0 z-30">
      <div className="flex items-center gap-4">
        {onMenuToggle && !isCustomer && !isVendor && !isPersonnel && (
      ...
      <div className="flex items-center gap-3">
        ...
        <div className={`items-center gap-3 ${(isCustomer || isVendor || isPersonnel) ? 'hidden lg:flex' : 'flex'}`}>
        ...
        {/* Vendor Mobile Menu Trigger */}
        {isVendor && (
          <div className="flex lg:hidden items-center">
            <button 
              onClick={() => setVendorMobileMenuOpen(true)}
              className="p-2.5 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors relative"
            >
              <Menu className="w-6 h-6 text-slate-800 dark:text-slate-200" />
              {notifications.filter(n => !(n.is_read || n.read)).length > 0 && (
            ...
            <VendorMobileProfileModal
              isOpen={vendorMobileMenuOpen}
              onClose={() => setVendorMobileMenuOpen(false)}
              profile={profile}
              isDark={isDark}
              toggleTheme={toggleTheme}
              unreadNotificationsCount={notifications.filter(n => !(n.is_read || n.read)).length}
        ...
        {/* Personnel Mobile Menu Trigger */}
        {isPersonnel && (
          <div className="flex lg:hidden items-center">
            <button 
              onClick={() => setPersonnelMobileMenuOpen(true)}
              className="p-2.5 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors relative"
            >
              <Menu className="w-6 h-6 text-slate-800 dark:text-slate-200" />
              {notifications.filter(n => !(n.is_read || n.read)).length > 0 && (
            ...
            <PersonnelMobileProfileModal
              isOpen={personnelMobileMenuOpen}
              onClose={() => setPersonnelMobileMenuOpen(false)}
              profile={profile}
              isDark={isDark}
              toggleTheme={toggleTheme}
              unreadNotificationsCount={notifications.filter(n => !(n.is_read || n.read)).length}
